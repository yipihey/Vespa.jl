#!/usr/bin/env bash
# CONTROLLED single-step test (RAMSES, f32 GPU): same CICASS realization as the Enzo 1-step run
# (deterministic generate), dump full cell state at the IC (z=1000) and after ONE fixed step
# (z=990, capped). Compare to enzo_*_1s to confirm identical ICs + isolate per-step divergence.
...
export CIC_ZOUT="1000,990" CIC_CHEM=1 CIC_CHEM_ENGINE=kernels CIC_CHEM_BACKEND=cuda CIC_DUAL_ENERGY=1 CIC_COMPTON_DRAG=1 CIC_XSPEC=1 CIC_CELLCMP=1 CIC_TAG=_1s
LOG=reports/multicode/c128_ramses_1step.log
