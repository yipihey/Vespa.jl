...
@testset "MUSCL-Hancock 3-D driver — PPM reconstruction" begin
    ...
    # ── D. HLLC Riemann option (riemann=:hllc): conserves + differs from HLL ─────
    @testset "riemann=:hllc conserves + differs from :hll" begin
        ...
        for s in 1:3
            PPMKernels.muscl_hancock_step_3d!(st..., cdims, ng; dt = dt, gamma = gamma, dx = dx,
                                              order = isodd(s) ? (1, 2, 3) : (3, 2, 1), recon = :ppm, riemann = :hllc)
        end
        @test abs(tot(st[1]) - m0) / abs(m0) < 1e-12          # HLLC conserves mass
        @test abs(tot(st[5]) - e0) / abs(e0) < 1e-12          # …and energy
        # HLLC ≠ HLL (contact-resolving ⇒ a genuinely different, less diffusive flux)
        hll = stage(); hllc = stage()
        PPMKernels.muscl_hancock_step_3d!(hll..., cdims, ng; dt = dt, gamma = gamma, dx = dx, recon = :ppm, riemann = :hll)
        PPMKernels.muscl_hancock_step_3d!(hllc..., cdims, ng; dt = dt, gamma = gamma, dx = dx, recon = :ppm, riemann = :hllc)
        @test PPMKernels.to_host(hll[1]) != PPMKernels.to_host(hllc[1])
        # metal-f32 ≡ cpu-f32 on the HLLC path
        if metal_ready()
            run_d(nm) = begin
