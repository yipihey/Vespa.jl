...
@kernel function step_ref_kernel!(o1,o2,o3,o4,o5,o6,o7,o8,o9,
        ...
        smallr::T, pfl::T, llf_dmin::T, llf_pmin::T) where {T}
    ...
    @inbounds begin
        ...
        Fxl = riemann(_facep(mhxm,δRxm), _facep(mhc,δLxc), 1, γ,ch,smallr,pfl,llf_dmin,llf_pmin,rsol)
        Fxh = riemann(_facep(mhc,δRxc),  _facep(mhxp,δLxp),1, γ,ch,smallr,pfl,llf_dmin,llf_pmin,rsol)
        # y faces
        ...
        Fyl = riemann(_facep(mhym,δRym), _facep(mhc,δLyc), 2, γ,ch,smallr,pfl,llf_dmin,llf_pmin,rsol)
        Fyh = riemann(_facep(mhc,δRyc),  _facep(mhyp,δLyp),2, γ,ch,smallr,pfl,llf_dmin,llf_pmin,rsol)
        # z faces
        ...
        Fzl = riemann(_facep(mhzm,δRzm), _facep(mhc,δLzc), 3, γ,ch,smallr,pfl,llf_dmin,llf_pmin,rsol)
        Fzh = riemann(_facep(mhc,δRzc),  _facep(mhzp,δLzp),3, γ,ch,smallr,pfl,llf_dmin,llf_pmin,rsol)
...
function step_ref!(s::MHDState{T}, dt::Real; ch::Real, decay::Real) where {T}
    ...
    step_ref_kernel!(s.be, 256)(s.scratch..., s.U..., Nx,Ny,Nz, cx,cy,cz, rec, rsol, dtdx, s.γ, T(ch), T(decay),
                                s.smallr, s.pfl, s.llf_dmin, s.llf_pmin; ndrange = ncells(s))
    KA.synchronize(s.be)
