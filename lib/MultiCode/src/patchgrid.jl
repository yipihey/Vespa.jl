...
# `:metal` backend; the Metal ext implements it.  `sys` is the FVGK dual-energy system, `gesc` the
# GE_SCALE, `rec`/`riem` the reconstruction/Riemann symbols.
function _fvgk_build_metal_grid end
_fvgk_metal_step!(g, dtf::Float32, sigspeed, dx::Float32) =
    error("solver=:fvgk on the :metal backend needs `using Metal` (Apple Silicon) to load MultiCodeFVGKMetalExt")
_fvgk_metal_grav_kick!(g, accel; dx, halfdt) =
    error("solver=:fvgk on the :metal backend needs `using Metal` (Apple Silicon) to load MultiCodeFVGKMetalExt")
_fvgk_metal_synchronize!(g) = nothing
...
@kernel function _chem_interior_k!(D, Ge, Tau, sp1, sp2, sp3,
                                   du, vu2, tu, dt, z, hub, omc, olc, deut, rtab, ctab,
    ...
    @inbounds begin
        ...
        en, hn, h2n, hdn, _ = ChemistryKernels.evolve_cell(d*du, e0*vu2, h1*du, h2*du,
                                  ...
                                  hubble=hub, Om=omc, OL=olc, fh=0.76, deuterium=deut,
...
function _chem_interior!(pg::PatchGrid, p::Patch, a_value, dt, du, lu, tu, rate_tables, cool_tables,
    ...
    _chem_interior_k!(pg.backend)(p.D, p.Ge, p.Tau, p.species[1], p.species[2], sp3,
        ...
        Float64(hub), Float64(omc), Float64(olc),
...
@inline function _recomb_compton_cell(d, e0, hii, dt, z, Hz, fh, gamma, nsub::Int)
    R = typeof(e0)
    ...
    hii_c = hii; e_c = e0; sdt = dt/R(nsub)
    # JOINT closed form per step (no operator-split bias):
    #   Compton:  T(t)=Tγ+(T₀-Tγ)e^{-t/t_C}  (exact for x≈const over a step; recomb is slow per step here)
    #   Recomb :  evaluate k₂ at the TIME-AVERAGED T of that relaxation, T̄=Tγ+(T₀-Tγ)·(1-e^{-Δt/t_C})/(Δt/t_C),
    ...
    gm1 = R(gamma) - one(R)
    for _ in 1:nsub
        nHI   = max((R(fh)*d - hii_c)/mh, R(1e-30))                    # HII-only color; H2/HD not carried
        nHII0 = hii_c/mh
...
function _chem_analytic!(pg::PatchGrid, p::Patch, a_value, dt, du, lu, tu, hz, nsub::Int)
    ...
    # The ENV branch resolves to a CONCRETE type LITERAL here, then dispatches to the type-parametrized
    # launcher below.  This keeps the kernel launch statically typed ⇒ inferable AND precompilable — a
    # runtime-typed `R` (as before) makes the launch a dynamic dispatch the pkgimage can't cache.
    if pg.besym === :metal || get(ENV, "CIC_CHEM_F32", "0") == "1"
        _chem_analytic_typed!(Float32, pg, p, a_value, dt, du, lu, tu, hz, nsub)
    else
        _chem_analytic_typed!(Float64, pg, p, a_value, dt, du, lu, tu, hz, nsub)
    end
end
function _chem_analytic_typed!(::Type{R}, pg::PatchGrid, p::Patch, a_value, dt, du, lu, tu, hz, nsub::Int) where {R}
    nd1, nd2, _ = pg.nd; pd1, pd2, pd3 = pg.pdim
...
function patch_step!(pg::PatchGrid, dt::Real; a_value::Real, order=(1,2,3),
    ...
    PPMKernels.with_pool() do
      if do_hydro
        ...
        # ONE cosmology routine for every chem path: H(z) from ChemistryKernels.hubble_z_of with the
        # ACTUAL (cosmo_h0,Om,OL) — the analytic channel and the stiff network read the identical Hz,
        # so the recombination freeze-out (Peebles C ∝ 1/Hz, radiation-inclusive) can never diverge.
        if do_chem && chem && !isempty(pg.patches[1].species) && chemmode === :analytic
            hz = ChemistryKernels.hubble_z_of(1.0/a_value - 1.0; hubble=cosmo_h0, Om=cosmo_Om, OL=cosmo_OL)
            for p in pg.patches
