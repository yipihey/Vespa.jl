...
function global_gravity_gpu(pg::PatchGrid; G::Real=1.0, a::Real=1.0, boxsize::Real=1.0,
    ...
    # CIC_GRAV_KAFFT=1: solve with the in-tree KA FFT (no cuFFT).  cuFFT's R2C plan needs a huge
    # OUT-OF-POOL work area (~18 B/cell → the 900³ ceiling); the KA FFT uses only small POOLABLE scratch.
    ...
    if kafft
        ...
        PoissonKernels.fft_poisson_rfft!(φd, ρd; G=G, a=a, boxsize=boxsize)   # cuFFT rfft, any size; φd may === ρd
    end
