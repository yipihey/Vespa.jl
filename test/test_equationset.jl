...
struct IdealHydroEF <: Vespa.EquationSet
    γ::Float64
end
@inline _to_std(U)   = (U[2], U[3], U[4], U[5], U[1])   # EF → standard order
@inline _from_std(U) = (U[5], U[1], U[2], U[3], U[4])   # standard → EF order

Vespa.nvars(::IdealHydroEF) = 5
Vespa.conserved_names(::IdealHydroEF) =
    (:total_energy, :density, :momentum_x, :momentum_y, :momentum_z)
Vespa.density_index(::IdealHydroEF) = 2
Vespa.momentum_indices(::IdealHydroEF) = (3, 4, 5)
Vespa.energy_index(::IdealHydroEF) = 1
Vespa.cons2prim(m::IdealHydroEF, U) = _from_std(Vespa.cons2prim(_to_std(U), m.γ))
Vespa.prim2cons(m::IdealHydroEF, W) = _from_std(Vespa.prim2cons(_to_std(W), m.γ))
Vespa.sound_speed(m::IdealHydroEF, W) = Vespa.sound_speed(_to_std(W), m.γ)
Vespa.riemann_flux(m::IdealHydroEF, WL, WR, axis::Int) =
    _from_std(Vespa.hllc_flux(_to_std(WL), _to_std(WR), m.γ, axis))
